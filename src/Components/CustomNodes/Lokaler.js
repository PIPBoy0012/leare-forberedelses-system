...
export function LokaleNode({ data }) {
  ...
  return (
    <div className="Customnode rounded-b-lg border truncate border-black ">
      ...
      <button
        /*onClick={SaveNodeIntoDB()} */ className="gemknap absolute text-transparent text-xs bg-transparent w-16 h-5 z-4 rounded-md hover:bg-slate-500 hover:text-black"
      >
        Gem Node
      </button>
      <textarea
        rows="1"
        className="Customnodeinput resize-none text-xs"
        name="beskrivelse"
        onChange={onChange}
      />
